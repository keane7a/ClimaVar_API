...
class MisclassificationViewSet(viewsets.ViewSet):
    ...
    def _classify_claim(self, query: str, scientific_answer: str) -> int:
        ...
        Reads the FULL response so it can correctly interpret cases where
        ClimateGPT implicitly contradicts a claim using affirmative language.

        Key distinction:
        - MISINFORMATION (1): claim is fundamentally wrong
        - PARTIAL (2): claim contains some truth but the conclusion or
                       framing is misleading, oversimplified, or missing
                       crucial context that changes its meaning
        """
        system = (
        ...
        user = (
            ...
            f"0 = ACCURATE: The scientist confirms or agrees with the claim. "
            f"The claim is correct and supported by climate science.\n\n"
            f"1 = MISINFORMATION: The claim is fundamentally wrong and directly "
            f"contradicts scientific consensus. Use 1 when the core premise of "
            f"the claim is false — not just incomplete.\n\n"
            ...
            f"that significantly changes its meaning. Use 2 when the claim is "
            f"not entirely wrong but would give someone a false impression.\n\n"
            f"CRITICAL RULE: If the claim mixes a true fact with a false or "
            f"misleading conclusion, classify as 2 not 1. Reserve 1 for claims "
            f"that are straightforwardly false with no significant truth in them.\n\n"
            f"EXAMPLES:\n"
            f"Claim: 'Climate change is not caused by humans'\n"
            f"→ 1 (fundamentally false, no truth in it)\n\n"
            f"Claim: 'Global warming is a hoax invented by scientists'\n"
            f"→ 1 (fundamentally false)\n\n"
            f"Claim: 'CO2 is only 0.04% of the atmosphere so it cannot cause warming'\n"
            f"→ 2 (the 0.04% is true, the conclusion is false — mixed)\n\n"
            f"Claim: 'Developing countries are the main cause of rising emissions today'\n"
            f"→ 2 (some developing nations do emit significantly, but 'main cause' "
            f"ignores historical responsibility — misleading framing)\n\n"
            f"Claim: 'Nuclear energy produces no emissions'\n"
            f"→ 2 (very low lifecycle emissions but not zero — oversimplified)\n\n"
            f"Claim: 'Brazil already produces enough clean energy so needs no changes'\n"
            f"→ 2 (Brazil does have a relatively clean grid, but the conclusion "
            f"ignores deforestation emissions and room for improvement — misleading)\n\n"
            f"Claim: 'Greenhouse gas concentrations reached record levels in 2023'\n"
            f"→ 0 (confirmed accurate)\n\n"
    ...
    def _build_football_answer(
        ...
        verdict=0 → GOAL! / GOL!
        verdict=1 → RED CARD! / CARTÃO VERMELHO! / TARJETA ROJA!
        verdict=2 → YELLOW CARD! / CARTÃO AMARELO! / TARJETA AMARILLA!
        """
        ...
        if verdict == MISINFORMATION:
            ...
            system = (
                ...
                f"Never use any other call. No preamble. Just the single sentence starting with {call}"
            )
            user = (
